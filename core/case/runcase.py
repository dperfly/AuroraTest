...
class AsyncRunCase:
    def __init__(self, raw_data, cache, hook_func,select_cases:SelectCase = None):
        ...
        info_log(f"Barrels:{self.barrels.items()}")
        self.sorted_orders = sorted(self.barrels.keys(), reverse=True)
        info_log(f"sorted_orders:{self.sorted_orders}")
        self.all_case_map = self.t_c_a_g.get_all_case_map()
        self.global_semaphore = asyncio.Semaphore(5)  # 全局并发控制
        info_log(f"Execution order - Barrels: {self.barrels.items()}")
        info_log(f"Sorted priority orders: {self.sorted_orders}")
        ...
        try:
            async with self.global_semaphore:
                if case := self.all_case_map.get(case_name):
                    info_log(f"Running case: {case_name}")
